...
Elm.Counter.make = function (_elm) {
   ...
   var cps = function (model) {
      return model.store.cursor.nb * model.store.cursor.production + model.store.granma.nb * model.store.granma.production + model.store.farmer.nb * model.store.farmer.production;
   };
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.div,
      _U.list([countStyle]),
      _U.list([$Html.text($String.concat(_U.list([" Cookies: ",$Basics.toString($Basics.round(model.cookies)),"\nCpS: ",$Basics.toString(cps(model))])))]))]));
   });
   var generateCookies = function (model) {    return {cookies: model.cookies + cps(model) * 0.1,store: model.store};};
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "Increment": return {ctor: "_Tuple2",_0: {cookies: model.cookies + _p0._0,store: model.store},_1: $Effects.none};
         case "Decrement": return {ctor: "_Tuple2",_0: {cookies: model.cookies + _p0._0,store: model.store},_1: $Effects.none};
         case "Update": return {ctor: "_Tuple2",_0: generateCookies(model),_1: $Effects.none};
         default: var _p1 = _p0._0;
           switch (_p1.ctor)
           {case "Cursor": return _U.cmp(model.cookies,model.store.cursor.price) < 0 ? {ctor: "_Tuple2",_0: model,_1: $Effects.none} : {ctor: "_Tuple2"
                                                                                                                                       ,_0: {cookies: model.cookies - model.store.cursor.price
                                                                                                                                            ,store: {cursor: {nb: model.store.cursor.nb + 1
                                                                                                                                       ...
                                                                                                                                       ,_1: $Effects.none};
              case "Granma": return _U.cmp(model.cookies,model.store.granma.price) < 0 ? {ctor: "_Tuple2",_0: model,_1: $Effects.none} : {ctor: "_Tuple2"
                                                                                                                                         ,_0: {cookies: model.cookies - model.store.granma.price
                                                                                                                                              ,store: {cursor: model.store.cursor
                                                                                                                                                      ,granma: {nb: model.store.granma.nb + 1
                                                                                                                                                      ...
                                                                                                                                                      ,farmer: model.store.farmer}}
                                                                                                                                         ,_1: $Effects.none};
              default: return _U.cmp(model.cookies,model.store.farmer.price) < 0 ? {ctor: "_Tuple2",_0: model,_1: $Effects.none} : {ctor: "_Tuple2"
                                                                                                                                   ,_0: {cookies: model.cookies - model.store.farmer.price
                                                                                                                                        ,store: {cursor: model.store.cursor
                                                                                                                                                ,granma: model.store.granma
                                                                                                                                                ,farmer: {nb: model.store.farmer.nb + 1
   ...
   var Farmer = {ctor: "Farmer"};
   var Granma = {ctor: "Granma"};
   var Cursor = {ctor: "Cursor"};
   var Model = F2(function (a,b) {    return {cookies: a,store: b};});
   var Store = F3(function (a,b,c) {    return {cursor: a,granma: b,farmer: c};});
   var StoreItem = F4(function (a,b,c,d) {    return {nb: a,production: b,price: c,growth: d};});
...
Elm.Cookie.make = function (_elm) {
   ...
   if (_elm.Cookie.values) return _elm.Cookie.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("cookie")]),
      _U.list([$Html.fromElement(A3($Graphics$Collage.collage,
      200,
      200,
      _U.list([$Graphics$Collage.toForm(A2($Graphics$Input.clickable,
      A2($Signal.message,address,$Counter.Increment(1)),
      A3($Graphics$Element.image,200,200,"/img/cookie.jpg")))])))]));
   });
   return _elm.Cookie.values = {_op: _op,view: view};
};
Elm.Market = Elm.Market || {};
Elm.Market.make = function (_elm) {
   "use strict";
   _elm.Market = _elm.Market || {};
   if (_elm.Market.values) return _elm.Market.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Counter = Elm.Counter.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var regOnClick = F2(function (address,item) {    return A2($Html$Events.onClick,address,$Counter.Buy(item));});
   var view = F2(function (address,model) {
      return A2($Html.ul,
      _U.list([]),
      _U.list([A2($Html.li,
              ...
              _U.list([A2($Html.button,
                      _U.list([A2(regOnClick,address,$Counter.Cursor)]),
                      _U.list([$Html.text($String.concat(_U.list(["Cursor ",$Basics.toString(model.store.cursor.nb)])))]))
                      ,$Html.text($String.concat(_U.list(["Price: ",$Basics.toString(model.store.cursor.price)])))]))
              ,A2($Html.li,
              _U.list([]),
              _U.list([A2($Html.button,
                      _U.list([A2(regOnClick,address,$Counter.Granma)]),
                      _U.list([$Html.text($String.concat(_U.list(["Granma ",$Basics.toString(model.store.granma.nb)])))]))
                      ,$Html.text($String.concat(_U.list(["Price: ",$Basics.toString(model.store.granma.price)])))]))
              ,A2($Html.li,
              _U.list([]),
              _U.list([A2($Html.button,
                      _U.list([A2(regOnClick,address,$Counter.Farmer)]),
                      _U.list([$Html.text($String.concat(_U.list(["Farmer ",$Basics.toString(model.store.farmer.nb)])))]))
                      ,$Html.text($String.concat(_U.list(["Price: ",$Basics.toString(model.store.farmer.price)])))]))]));
   });
   return _elm.Market.values = {_op: _op,view: view,regOnClick: regOnClick};
};
...
Elm.Layout.make = function (_elm) {
   ...
   $Html = Elm.Html.make(_elm),
   $List = Elm.List.make(_elm),
   $Market = Elm.Market.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var view = F2(function (address,model) {
      return A2($Html.div,_U.list([]),_U.list([A2($Counter.view,address,model),A2($Market.view,address,model),A2($Cookie.view,address,model)]));
   });
   return _elm.Layout.values = {_op: _op,view: view};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var timeUpdate = A2($Signal.map,$Counter.Update,$Time.every(100 * $Time.millisecond));
   var init = {ctor: "_Tuple2"
              ,_0: {cookies: 0.0
                   ,store: {cursor: {nb: 0.0,production: 1.0,price: 10.0,growth: 1.0}
                           ,granma: {nb: 0.0,production: 5.0,price: 50.0,growth: 5.0}
                           ,farmer: {nb: 0.0,production: 20.0,price: 500.0,growth: 50.0}}}
              ,_1: $Effects.none};
   var app = $StartApp.start({init: init,view: $Layout.view,update: $Counter.update,inputs: _U.list([timeUpdate])});
   var main = app.html;
   return _elm.Main.values = {_op: _op,init: init,timeUpdate: timeUpdate,app: app,main: main};
};
